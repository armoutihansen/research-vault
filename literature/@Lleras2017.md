...
- **Indistinguishability result.** Any competition filter can be rewritten as both a categorization shading relation $\Gamma_{\sqsupset}$ ([[@Manzini2012a|Manzini–Mariotti 2012]]) and a set of rationales $\Gamma_{\{R_i\}}$ ([[@Cherepanov2013|Cherepanov et al. 2013]]); the three first stages generate identical choice behavior and are inseparable on choice data alone.
...
- The authors note that combining their competition filter with the [[@Masatlioglu2012|Masatlioglu et al. (2012)]] attention filter (consideration sets satisfying *both*) does **not** yield a clean characterization; they state the result exists "upon request" but is omitted for space — an explicit open modeling problem.
